...
func TestResolveDriverMetaRegistry(t *testing.T) {
	testCases := map[string]interface{}{
		"resolve named registry from daemon config": []interface{}{
			&Service{config: &config.Config{Staged: &config.DataSet{Drivers: map[string]interface{}{
	...
	for msg, tc := range testCases {
		func(c []interface{}) {
			resolvedMeta, err := c[0].(*Service).resolveDriverMeta(c[1].(map[string]interface{}))
			if len(c[2].(string)) > 0 {
