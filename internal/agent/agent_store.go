...
// Static errors for resolveOrRecreateConvoState (exported for external callers).
var (
...
type AgentStore interface {
	StartInference(msg *dipper.Message) error
...
// It returns an error to allow callers (handleAgentStart) to emit an error response on the
// unrecoverable path (evicted ConvoState + no agent_name label).
func (p *PersistentAgentStore) StartInference(msg *dipper.Message) error {
	p.wg.Add(1)
	defer p.wg.Done()

	resumeKey := msg.Labels["resume_key"]
	defer dipper.SafeExitOnError("[agent] error in StartInference", func(r interface{}) {
	...
	// Resolve or recreate ConvoState using the shared helper.
	cs, created, err := p.resolveOrRecreateConvoState(convoID, agentName, agentOverride)
	if err != nil {
		p.Errorf("[agent] StartInference: %v", err)

		return err
	}

	// Ensure we have an agent name to run the turn.
	resolvedAgentName := ""
	if cs.Agent != nil {
		resolvedAgentName = cs.Agent.Name
	}
	...
	// Build the session message for initNewSession.
	// We need to pass the payload through so that text, user, engine, driver, etc. are available.
	s := &AgentSession{}
	...
	payloadConvoID, _ := dipper.GetMapDataStr(msg.Payload, "convo_id")
	if payloadConvoID == "" {
		payloadConvoID = s.ConvoID
	}

	// Take the turn lock after setup so lockTurn can use s.Agent.TurnLockTimeout.
	s.lockTurn(p, payloadConvoID)
	...
	// Log whether we recreated or used existing state.
	if created {
		p.Infof("[agent] StartInference convo=%s agent=%s (recreated)", payloadConvoID, resolvedAgentName)
	} else {
		p.Infof("[agent] StartInference convo=%s agent=%s", payloadConvoID, resolvedAgentName)
	}
