...
func TestStartInference_ConvoStateEvicted_NoAgent_ReturnsError(t *testing.T) {
	helper := newMemCacheHelper()
	convoID := "evicted-inference-convo-1"
	...
	err := store.StartInference(msg)
	store.Wait()

	// Assertion 1: StartInference returns an error (unrecoverable).
	require.Error(t, err, "StartInference must return error when convo evicted and no agent supplied")
	require.Contains(t, err.Error(), "conversation "+convoID+" expired and no agent supplied to recreate")
	...
	// Assertion 4: no model/AI driver call nor agent_response emitted (no session).
	require.Empty(t, helper.getEmitted(), "fixed behavior must not emit any message")
...
// TestStartInference_ConvoStateEvicted_WithAgent_Recovers asserts that when
// the ConvoState has been evicted but an agent_name label IS provided,
// StartInference recreates the ConvoState with the supplied agent and proceeds
// to start a session.
func TestStartInference_ConvoStateEvicted_WithAgent_Recovers(t *testing.T) {
	helper := newMemCacheHelper()
	convoID := "evicted-inference-convo-2"
	...
	// Seed the agent config so the supplied agent can be resolved.
	helper.cfg.DataSet.Agents["recovery_agent"] = config.Agent{
		Name: "recovery_agent",
	}
	...
	// Drive the recovery path WITH agent_name supplied.
	msg := &dipper.Message{
		Labels: map[string]string{
			"resume_key": "test-resume-key-recover",
			"convo_id":   convoID,
			"agent_name": "recovery_agent", // Agent supplied for recovery
		},
		Payload: map[string]interface{}{
			"text": "hello again",
		},
	...
	// Assertion 1: No error returned (recovery succeeded).
	require.NoError(t, err, "StartInference must succeed when agent_name supplied for evicted convo")

	// Assertion 2: ConvoState was recreated with the new agent (persisted to cache).
	csData, ok := helper.getCache(ConvoStateKeyPrefix + convoID)
	require.True(t, ok, "convo_state must be recreated")
	var cs ConvoState
	require.NoError(t, json.Unmarshal([]byte(csData), &cs))
	require.Equal(t, "recovery_agent", cs.Agent.Name, "recreated ConvoState must have the supplied agent")

	// Assertion 3: agent_response emitted with session ID (session started).
	emitted := helper.getEmitted()
	require.Len(t, emitted, 1, "agent_response must be emitted")
	resp := emitted[0]
	require.Equal(t, "agent_response", resp.Subject)
	require.Equal(t, "test-resume-key-recover", resp.Labels["resume_key"])
	require.NotEmpty(t, resp.Labels["agent_session_id"], "agent_session_id must be set")
}

// TestStartInference_ConvoStatePresent_NoOverride_UsesExisting asserts the
// happy path: when a live ConvoState exists and the message does NOT supply
// an agent_name override, the existing state (and its resolved agent) is used.
func TestStartInference_ConvoStatePresent_NoOverride_UsesExisting(t *testing.T) {
	helper := newMemCacheHelper()
	convoID := "live-inference-convo-1"

	// Seed a live ConvoState with an existing agent (using ChatTurn type so agent resolves).
	seeded := &ConvoState{
		ConvoID: convoID,
		LastSession: &ConvoSessionRef{
			SessionID: "sess-live",
			AgentName: "existing_agent",
			Type:      AgentSessionTypeChatTurn, // Must be ChatTurn for agent resolution
			Status:    ConvoSessionStatusComplete,
		},
	}
	helper.cache[ConvoStateKeyPrefix+convoID] = string(dipper.SerializeContent(seeded))

	// Seed the agent config for the existing agent.
	helper.cfg.DataSet.Agents["existing_agent"] = config.Agent{
		Name: "existing_agent",
	}
	...
	// Call StartInference WITHOUT agent_name override.
	msg := &dipper.Message{
		Labels: map[string]string{
			"resume_key": "test-resume-key-existing",
			"convo_id":   convoID,
			// No agent_name, no agent_override
		},
		Payload: map[string]interface{}{
	...
	// Assertion 1: No error (proceeded with existing state).
	require.NoError(t, err, "StartInference must succeed with existing ConvoState")

	// Assertion 2: agent_response emitted (session started successfully).
	emitted := helper.getEmitted()
	require.Len(t, emitted, 1, "agent_response must be emitted")
	resp := emitted[0]
	require.Equal(t, "agent_response", resp.Subject)
	require.Equal(t, "test-resume-key-existing", resp.Labels["resume_key"])
	require.NotEmpty(t, resp.Labels["agent_session_id"], "agent_session_id must be set")

	// Assertion 3: The guard log for "cannot determine agent name" is NOT present.
	require.NotContains(t, logBuf.String(), "cannot determine agent name for convo "+convoID,
		"present convo_state must resolve an agent and skip the guard")
}

// TestStartInference_ConvoStatePresent_Override_Recreates asserts that when
// a live ConvoState exists but the message supplies agent_name with
// agent_override=true, the ConvoState is recreated/overwritten with the new
// agent.
func TestStartInference_ConvoStatePresent_Override_Recreates(t *testing.T) {
	helper := newMemCacheHelper()
	convoID := "live-inference-convo-2"

	// Seed a live ConvoState with an existing agent (using ChatTurn type).
	seeded := &ConvoState{
		ConvoID: convoID,
		LastSession: &ConvoSessionRef{
			SessionID: "sess-live",
	...
	// Seed both agent configs.
	helper.cfg.DataSet.Agents["old_agent"] = config.Agent{Name: "old_agent"}
	helper.cfg.DataSet.Agents["new_agent"] = config.Agent{Name: "new_agent"}
	...
	// Call StartInference WITH agent_name AND agent_override=true.
	msg := &dipper.Message{
		Labels: map[string]string{
			"resume_key":     "test-resume-key-override",
			"convo_id":       convoID,
			"agent_name":     "new_agent",
			"agent_override": "true", // Override!
		},
		Payload: map[string]interface{}{
			"text": "switch agent",
		},
	...
	require.NoError(t, err, "StartInference must succeed with override")
	...
	var cs ConvoState
	require.NoError(t, json.Unmarshal([]byte(csData), &cs))
	require.Equal(t, "new_agent", cs.Agent.Name, "ConvoState must be recreated with new agent")

	// Assertion 3: agent_response emitted with success.
	emitted := helper.getEmitted()
	require.Len(t, emitted, 1)
	resp := emitted[0]
	require.Equal(t, "agent_response", resp.Subject)
	require.Equal(t, "test-resume-key-override", resp.Labels["resume_key"])
	require.NotEmpty(t, resp.Labels["agent_session_id"], "agent_session_id must be set")
}

// TestStartInference_ConvoStatePresent_WithAgentNoOverride_Sticks asserts
// that when a live ConvoState exists and the message supplies a DIFFERENT
// agent_name but agent_override=false (or absent), the existing state is
// preserved (the supplied agent is ignored).
func TestStartInference_ConvoStatePresent_WithAgentNoOverride_Sticks(t *testing.T) {
	helper := newMemCacheHelper()
	convoID := "live-inference-convo-3"
	...
	// Seed both agent configs.
	helper.cfg.DataSet.Agents["sticky_agent"] = config.Agent{Name: "sticky_agent"}
	helper.cfg.DataSet.Agents["different_agent"] = config.Agent{Name: "different_agent"}
	...
	// Call StartInference with a DIFFERENT agent_name but NO override (or override=false).
	msg := &dipper.Message{
		Labels: map[string]string{
			"resume_key":     "test-resume-key-sticky",
			"convo_id":       convoID,
			"agent_name":     "different_agent", // Different agent supplied
			"agent_override": "false",           // But override is false
		},
		Payload: map[string]interface{}{
			"text": "should stick",
		},
	...
	// Assertion 1: No error.
	require.NoError(t, err, "StartInference must succeed")

	// Assertion 2: agent_response emitted with success (existing agent used).
	emitted := helper.getEmitted()
	require.Len(t, emitted, 1)
	resp := emitted[0]
	require.Equal(t, "agent_response", resp.Subject)
	require.Equal(t, "test-resume-key-sticky", resp.Labels["resume_key"])
	require.NotEmpty(t, resp.Labels["agent_session_id"], "agent_session_id must be set")

	// Assertion 3: No recreation log (log captures ERROR only, so just verify session started).
	require.NotContains(t, logBuf.String(), "recreated", "log must NOT indicate recreation when override=false")
}

// ===== StartInference Recovery Tests (Phase 3) =====
// These mirror the StartTurn recovery truth-table tests but exercise the
// eventbus-driven inference path (StartInference).

// TestStartInference_ConvoStateEvicted_NoAgent_ReturnsError asserts the FIXED
// behavior: when a conversation's convo_state:<id> key has been reclaimed by
// Redis (TTL/eviction) and no agent is supplied, StartInference returns an
// error (does not silently succeed). No session starts, no history is
