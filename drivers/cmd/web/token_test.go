...
func TestGetToken(t *testing.T) {
	...
	var token string
	assert.NotPanicsf(t, func() { token = getToken("test1", nil) }, "should not panic when getting token")
	...
	assert.NotPanicsf(t, func() { token = getToken("test1", nil) }, "should not panic when getting token from cache")
	...
	assert.NotPanicsf(t, func() { token = getToken("test1", nil) }, "should not panic when refreshing token")
	...
	githubSource["_expiresAt"] = time.Now().Add(-time.Minute * 15)
	githubSource["key"] = ""
	assert.NotPanicsf(t, func() { token = getToken("test1", nil) }, "should not panic when refreshing token with _parsed_key")
	assert.Equalf(t, "foobar3", token, "should get a new token using _parsed_key")
}

func TestGetTokenInstallationIDOverride(t *testing.T) {
	defer gock.Off()
	...
	params := map[string]interface{}{"installation_id": "456"}
	token := getToken("test1", params)
	assert.Equal(t, "override-token", token)
	assert.Equal(t, "123", githubSource["installation_id"], "base source must remain unchanged")
}
